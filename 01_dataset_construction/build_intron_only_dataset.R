# Build intron-only training sequences for one species.
#
# Positive sequences contain the introns of a representative transcript in
# native order, joined by five Ns. Negative sequences use the same introns in
# a shuffled order. This dataset is used by the intron-only feature ablation.
#
# Usage:
#   Rscript build_intron_only_dataset.R <species_index> [intron_least]
# Output:
#   <species>_intron_only.csv
...
OUT_DIR <- "/home/litianyi/Regulatory_region_translation/intron_only"
...
for (j in seq_along(txids)) {
  ...
  introns_seq <- sapply(is, function(gr) {
    as.character(subseq(fa[[as.character(seqnames(gr))]],
                        start(gr), end(gr)))
  })
...
data$spe <- SPECIES[i]
write.csv(data, file.path(OUT_DIR, paste0(SPECIES[i], "_intron_only.csv")),
          row.names = FALSE)
